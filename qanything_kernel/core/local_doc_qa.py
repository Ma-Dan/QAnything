...
class LocalDocQA:
    def __init__(self):
        ...
        self.mode: str = None
        self.local_rerank_service_url = "http://127.0.0.1:7861"
        self.ocr_url = 'http://127.0.0.1:7861/ocr'

    def get_ocr_result(self, image_data: dict):
        headers = {
            'Content-Type': 'application/json'
        }
        resp = requests.request("POST", "http://localhost:7861/ocr", headers=headers, data=json.dumps(image_data))
        #print(resp.text)
    ...
    def rerank_documents_for_local(self, query, source_documents):
        ...
        try:
            ...
            resp = requests.request("POST", "http://localhost:7861/reranker", headers=headers, data=json.dumps(reranker_data))
            print(resp.text)
